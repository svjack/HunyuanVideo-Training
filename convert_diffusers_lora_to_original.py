...
def convert_lora_sd(diffusers_lora_sd):
    ...
    for key in diffusers_lora_sd.keys():
        ...
        if key.startswith("context_embedder.token_refiner"):
            ...
            # just rename
            for k, v in token_refiner_patterns.items():
                if k in key:
                    new_key = key.replace(k, v).replace(
                        "context_embedder.token_refiner.refiner_blocks",
                        prefix + "txt_in.individual_token_refiner.blocks",
                    )
